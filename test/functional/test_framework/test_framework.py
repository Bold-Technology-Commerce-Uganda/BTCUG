...
TMPDIR_PREFIX = "BoldTechnologyCommerceUganda_func_test_"


class BoldTechnologyCommerceUgandaTestFramework():
    """Base class for a BoldTechnologyCommerceUganda test script.

    Individual BoldTechnologyCommerceUganda test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave BoldTechnologyCommerceUgandads and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop BoldTechnologyCommerceUgandads after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing BoldTechnologyCommerceUgandad/BoldTechnologyCommerceUganda-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use BoldTechnologyCommerceUganda-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: BoldTechnologyCommerceUgandads were not stopped and may still be running")
    ...
    def start_node(self, i, *args, **kwargs):
        """Start a BoldTechnologyCommerceUgandad"""
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple BoldTechnologyCommerceUgandads"""
    ...
    def stop_node(self, i):
        """Stop a BoldTechnologyCommerceUgandad test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple BoldTechnologyCommerceUgandad test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'BoldTechnologyCommerceUgandad exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "BoldTechnologyCommerceUgandad should have exited with an error"
                else:
                    assert_msg = "BoldTechnologyCommerceUgandad should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as BoldTechnologyCommerceUgandad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in BoldTechnologyCommerceUganda.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in BoldTechnologyCommerceUganda.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "BoldTechnologyCommerceUgandad"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run BoldTechnologyCommerceUgandads:
    ...
    ### BoldTechnologyCommerceUganda Specific TestFramework ###
    ###################################
    ...
    def get_prevouts(self, node_id, utxo_list, nHeight=-1):
        ...
        :return: prevouts:         ({bytes --> (int, bytes, int)} dictionary)
                                   maps CStake "uniqueness" (i.e. serialized COutPoint -or hash stake, for zBTCUG-)
                                   to (amount, prevScript, timeBlockFrom).
                                   For zBTCUG prevScript is replaced with serialHash hex string.
        """
    ...
    def make_txes(self, node_id, spendingPrevOuts, to_pubKey):
        ...
        for uniqueness in spendingPrevOuts:
            # spend BoldTechnologyCommerceUganda
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zBTCUG inputs when fDoubleSpend is false. It needs to
...
class ComparisonTestFramework(BoldTechnologyCommerceUgandaTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some BoldTechnologyCommerceUgandad binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "BoldTechnologyCommerceUgandad"),
                          help="BoldTechnologyCommerceUgandad binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "BoldTechnologyCommerceUgandad"),
                          help="BoldTechnologyCommerceUgandad binary to use for reference nodes (if any)")
