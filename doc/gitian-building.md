...
*Setup instructions for a gitian build of BOLDTECHNOLOGYCOMMERCEUGANDA Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BOLDTECHNOLOGYCOMMERCEUGANDA
...
to the BOLDTECHNOLOGYCOMMERCEUGANDA GitHub release page.
...
- [Building BOLDTECHNOLOGYCOMMERCEUGANDA Core](#building-BoldTechnologyCommerceUganda-core)
...
Building BOLDTECHNOLOGYCOMMERCEUGANDA Core
--------------------
...
```bash
gpg --output ${VERSION}-linux/${NAME}/BoldTechnologyCommerceUganda-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/BoldTechnologyCommerceUganda-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/BoldTechnologyCommerceUganda-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/BoldTechnologyCommerceUganda-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/BoldTechnologyCommerceUganda-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/BoldTechnologyCommerceUganda-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@btcugltd.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/BoldTechnologyCommerceUganda-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/BoldTechnologyCommerceUganda-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/BoldTechnologyCommerceUganda-osx-*-build.assert
```
