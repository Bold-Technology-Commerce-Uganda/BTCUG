...
- [Development guidelines](#development-guidelines)
    - [General BOLDTECHNOLOGYCOMMERCEUGANDA Core](#general-BoldTechnologyCommerceUganda-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BOLDTECHNOLOGYCOMMERCEUGANDA Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
BOLDTECHNOLOGYCOMMERCEUGANDA Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Bold-Technology-Commerce-Uganda/BTCUG/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_BoldTechnologyCommerceUganda
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_BoldTechnologyCommerceUganda --log_level=test_suite
$ valgrind -v --leak-check=full src/BoldTechnologyCommerceUgandad -printtoconsole
```
...
# A coverage report will now be accessible at `./test_BoldTechnologyCommerceUganda.coverage/index.html`.
```
...
However, in open source software such as BOLDTECHNOLOGYCOMMERCEUGANDA Core, where everyone uses
...
pay attention to for reviewers of BOLDTECHNOLOGYCOMMERCEUGANDA Core code.

General BOLDTECHNOLOGYCOMMERCEUGANDA Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BOLDTECHNOLOGYCOMMERCEUGANDA Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BOLDTECHNOLOGYCOMMERCEUGANDA Core so that they can be integrated
...
file descriptors it will cause problems with BOLDTECHNOLOGYCOMMERCEUGANDA's `select()` loop, because
...
```bash
$ lsof -p $(pidof BoldTechnologyCommerceUgandad) |\
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:BOLDTECHNOLOGYCOMMERCEUGANDA-Project/BOLDTECHNOLOGYCOMMERCEUGANDA.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `BoldTechnologyCommerceUganda-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `BoldTechnologyCommerceUganda-cli` and the GUI debug console use this table to determine how to
