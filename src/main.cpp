...
#if defined(NDEBUG)
#error "BoldTechnologyCommerceUganda cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uBTCUG) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("BoldTechnologyCommerceUganda-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Update BoldTechnologyCommerceUganda money supply
    nMoneySupply += (nValueOut - nValueIn);
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    // competitive BoldTechnologyCommerceUganda.
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // BoldTechnologyCommerceUganda: We use certain sporks during IBD, so check to see if they are
