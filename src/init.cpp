...
void PrepareShutdown()
{
    ...
    util::ThreadRename("BoldTechnologyCommerceUganda-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), BOLDTECHNOLOGYCOMMERCEUGANDA_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), BOLDTECHNOLOGYCOMMERCEUGANDA_PID_FILENAME));
#endif
...
//    strUsage += HelpMessageOpt("-hostip=<ip>", _("Specify default host IP for outbound connections"));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BoldTechnologyCommerceUganda specific functionality (Masternodes) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), BOLDTECHNOLOGYCOMMERCEUGANDA_MASTERNODE_CONF_FILENAME));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2024-%i The BoldTechnologyCommerceUganda Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("BoldTechnologyCommerceUganda-loadblk");
...
/** Sanity checks
 *  Ensure that BoldTechnologyCommerceUganda is running in a usable environment with all
...
#endif
    LogPrintf("BoldTechnologyCommerceUganda version %s (%s)\n", version_string, CLIENT_DATE);
}
...
/** Initialize BoldTechnologyCommerceUganda.
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. BoldTechnologyCommerceUganda is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single BoldTechnologyCommerceUganda process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. BoldTechnologyCommerceUganda is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //BoldTechnologyCommerceUganda specific: spork DB's
                ...
                // BoldTechnologyCommerceUganda: load previous sessions sporks if we have them.
