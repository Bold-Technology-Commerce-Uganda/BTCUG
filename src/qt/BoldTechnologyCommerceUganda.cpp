...
#include "config/BoldTechnologyCommerceUganda-config.h"
#endif
...
#include "qt/BoldTechnologyCommerceUganda/splash.h"
#include "qt/BoldTechnologyCommerceUganda/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("BoldTechnologyCommerceUganda-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in BoldTechnologyCommerceUganda.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in BoldTechnologyCommerceUganda.qrc)
...
/** Class encapsulating BTCUG blockchain startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (BOLDTECHNOLOGYCOMMERCEUGANDAGUI)
...
private:
    ...
    BOLDTECHNOLOGYCOMMERCEUGANDAGUI* window;
...
#include "BoldTechnologyCommerceUganda.moc"
...
{
    window = new BOLDTECHNOLOGYCOMMERCEUGANDAGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &BOLDTECHNOLOGYCOMMERCEUGANDAGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &BOLDTECHNOLOGYCOMMERCEUGANDAGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BOLDTECHNOLOGYCOMMERCEUGANDAGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BOLDTECHNOLOGYCOMMERCEUGANDAGUI::DEFAULT_WALLET);
        ...
        // BTCUG: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BOLDTECHNOLOGYCOMMERCEUGANDAGUI::handlePaymentRequest);
        connect(window, &BOLDTECHNOLOGYCOMMERCEUGANDAGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. BTCUG can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(BoldTechnologyCommerceUganda_locale);
    Q_INIT_RESOURCE(BoldTechnologyCommerceUganda);
    ...
    QFontDatabase::addApplicationFont(":/font/Azonix.ttf");
    QFontDatabase::addApplicationFont(":/font/GoodTiming.ttf");
    QFontDatabase::addApplicationFont(":/font/BankGothicLight.ttf");
    ...
    /// 6. Determine availability of data directory and parse BoldTechnologyCommerceUganda.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("BTCUG"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("BTCUG"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // BTCUG: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("BTCUG didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
