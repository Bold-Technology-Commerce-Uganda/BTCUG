...
#include "qt/BoldTechnologyCommerceUganda/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/BoldTechnologyCommerceUganda/forms/ui_topbar.h"
#include "qt/BoldTechnologyCommerceUganda/loadingdialog.h"
#include "qt/BoldTechnologyCommerceUganda/lockunlock.h"
#include "qt/BoldTechnologyCommerceUganda/qtutils.h"
#include "qt/BoldTechnologyCommerceUganda/receivedialog.h"
...
TopBar::TopBar(BOLDTECHNOLOGYCOMMERCEUGANDAGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelTitle3, ui->labelTitle4, ui->labelTitle5,
    ...
    // setCssProperty({ui->labelTitle1, ui->labelTitle7}, "text-title-topbar-2");
    setCssProperty({ui->labelTitle1, ui->labelTitle7}, "text-title-topbar");
    ...
    setCssProperty({ui->labelAmountTopPiv}, "amount-small-topbar-2");
    setCssProperty({ui->labelAvailablePiv}, "amount-topbar-2");
    setCssProperty({ui->labelPendingPiv, ui->labelImmaturePiv, ui->labelAmountPiv,
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open BoldTechnologyCommerceUganda.conf with default application"));
}
...
void TopBar::setNumConnections(int count)
{
    ...
    ui->pushButtonConnection->setButtonText(tr("%n connection(s)", "", count));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("BTCUG")
    updateDisplayUnit();
...
void TopBar::refreshStatus()
{
    ...
    case WalletModel::EncryptionStatus::Unencrypted:
        ui->pushButtonLock->setButtonText(tr("Unencrypted"));
        ui->pushButtonLock->setButtonClassStyle("cssClass", "btn-check-status-unlock", true);
        break;
    ...
    case WalletModel::EncryptionStatus::UnlockedForStaking:
        ui->pushButtonLock->setButtonText(tr("Unlocked for staking"));
        ui->pushButtonLock->setButtonClassStyle("cssClass", "btn-check-status-staking", true);
        break;
    case WalletModel::EncryptionStatus::Unlocked:
        ui->pushButtonLock->setButtonText(tr("Unlocked"));
        ui->pushButtonLock->setButtonClassStyle("cssClass", "btn-check-status-unlock", true);
        break;
    }
