...
#include "qt/BoldTechnologyCommerceUganda/forms/ui_snackbar.h"
#include "qt/BoldTechnologyCommerceUganda/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BOLDTECHNOLOGYCOMMERCEUGANDAGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BOLDTECHNOLOGYCOMMERCEUGANDAGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
